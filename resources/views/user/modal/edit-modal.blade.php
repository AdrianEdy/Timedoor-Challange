    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="POST" class="form-manage" id="form-edit"
                action="{{ route('update', Session::get('board')->id ?? old('editId')) }}"
                enctype="multipart/form-data">
                @csrf
                <input type="hidden" name="editId" value="{{ old('editId') ?? Session::get('board')->id }}">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        <label>Name</label>
                        <input type="text" class="form-control modal-name" name="editName"
                            value="{{ old('editName') ?? Session::get('board')->name }}">
                        @error('editName')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control modal-title" name="editTitle"
                            value="{{ old('editTitle') ?? Session::get('board')->title ?? null}}">
                        @error('editTitle')
                        <p class="small text-danger mt-5">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="form-group hidden">
                        ...
                        <input type="text" class="form-control" name="submitPass"
                            value="{{ Session::get('submitPass') ?? old('submitPass')}}">
                    </div>
                </div>
